 ...
 *
 */
public class ModuleTypeForbiddenView extends ViewPart {


	private Text searchText;
	private TableViewer tableViewer;
	
	public ModuleTypeForbiddenView() {
		    ...
		    public void keyReleased(KeyEvent e) {
		    	if (e.keyCode == 13) {  
		    		...
		    		if(searchString == null || searchString.trim().equals("")){
		    			tableViewer.setInput(Settings.heuristicModuleUnitStopMapList);
						tableViewer.refresh();
		    		}else{
		    			HeuristicModuleUnitStopMapList filteredHeuristicStopMapList
		    			= new HeuristicModuleUnitStopMapList();
		    			for(HeuristicModuleUnitStopMap map: Settings.heuristicModuleUnitStopMapList){
		    				if(map.getUnit().getName().toLowerCase().indexOf(searchString) != -1 ||
		    						map.getUnit().getFullQualifiedName().toLowerCase().indexOf(searchString) != -1 ||
				...
				if (o instanceof MenuItem) {
					...
					if (ti != null && ti.length > 0) { 
						for(int i = 0; i < ti.length; i++){
							Settings.heuristicModuleUnitStopMapList.removeMap((HeuristicModuleUnitStopMap) ti[i].getData());							
						}						
						tableViewer.setInput(Settings.heuristicModuleUnitStopMapList);
						tableViewer.refresh();
					}else{
	...
	private void createColumns(final Composite parent, final TableViewer viewer) {
		...
		col.setLabelProvider(new ColumnLabelProvider() {
			...
			public String getText(Object element) {
				HeuristicModuleUnitStopMap map = (HeuristicModuleUnitStopMap)element;
